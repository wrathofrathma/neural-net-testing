...
# Similar to load_one_hot_db, but I broke parse_to_onehot for that function so I could parse for just player 2. 
def load_one_hot_rewards(filename):
    file = open(filename,"r")
    lines = file.readlines()
    ...
    while(ln!=len(lines)):
        ...
        if(STATE==1):
            ...
            game_winner = game_info[1]
            game_data = [ lines[ln+1], lines[ln+2], lines[ln+3]]
            game_states, actions = parse_to_onehot(game_data)
            ...
            for i in range(len(game_states)):
                memory.add_sample((np.array(game_states[i]), reward, np.array(actions[i])))
    return memory
